...
$.fn.formValidator = function() {
	...
	var updateCSS = function(input,valid,required,action) {
		...
		var activated = ($(glyphicon_div).children('span.glyphicon-ok').is(":visible") || $(glyphicon_div).children('span.glyphicon-remove').is(":visible")); //See if user has touched this field yet
		var only_good = (!(activated) && action =='keyup'); // Only good is used so X's do not appear as a user begins typing on every input
		if (valid) {
			if ($(input).hasClass('input')) { //Only input fields get the status icons
...
$(document).ready(function() {
	var form_id = $(".validated-form").formValidator();
});
